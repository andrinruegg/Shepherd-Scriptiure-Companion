...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      ...
      <div className={`fixed md:relative inset-y-0 left-0 z-50 w-72 flex flex-col h-full glass-panel border-r-0 md:border-r rounded-r-3xl md:rounded-none transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} md:flex-shrink-0`}>
        <div className="p-5 border-b dark:border-white/5 pt-safe flex-shrink-0">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-4 space-y-1 no-scrollbar">
            ...
            chats.map((chat) => (
                ...
                <div className="flex-1 min-w-0">
                    {editingId === chat.id ? (
                    <form onSubmit={saveEdit} className="flex items-center gap-1" onClick={e => e.stopPropagation()}><input autoFocus type="text" value={editTitle} onChange={e => setEditTitle(e.target.value)} className="w-full bg-white dark:bg-slate-700 text-sm rounded px-1 py-0.5 border border-indigo-500 outline-none"/><button type="submit" className="text-emerald-500"><Check size={14}/></button></form>
