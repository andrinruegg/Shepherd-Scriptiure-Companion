...
const WorldExplorer: React.FC<{ language: string, onMenuClick: () => void }> = ({ language, onMenuClick }) => {
    ...
    const handleSendMessage = async () => {
        ...
        let acc = "";
        await sendMessageStream(messages, userText, undefined, 'NIV', language, 'Traveler', (chunk: string) => { acc += chunk; setMessages(prev => prev.map((m) => m.id === aiMsgId ? {...m, text: acc} : m)); }, () => setIsLoading(false), (error: any) => setIsLoading(false), `Role: ${activeNPC.persona}. Lang: ${language}`);
    };
