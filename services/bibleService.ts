...
export const fetchChapter = async (
    ...
    try {
        if (langCode.includes('roman') || langCode === 'ro') {
            ...
            if (fullBible && Array.isArray(fullBible)) {
                ...
                // Usually order matches exactly (0-65), but we verify with name normalization just in case
                let bookObj = fullBible.length === 66 ? fullBible[bookIndex] : null;
                
                if (!bookObj || superNormalize(bookObj.name) !== superNormalize(bookData.names.ro)) {
                    // Fallback to searching if direct index mismatch
            ...
            translationId = 'LUT';
            // Try specific GH repo for German Schlachter first
            try {
    ...
    // Fallback to AI if external sources failed or returned empty
...
async function fetchFromBolls(translationId: string, bookId: number, chapter: number): Promise<any[]> {
    try {
        ...
        console.warn("Bolls fetch failed", e);
    }
    return [];
}
