...
import { BIBLE_BOOKS, fetchChapter } from '../services/bibleService.ts';
import { BibleChapter, SavedItem, BibleHighlight } from '../types.ts';
...
const BibleReader: React.FC<BibleReaderProps> = ({ 
  ...
  const handleNext = () => { if (chapter < selectedBook.chapters) setChapter(c => c + 1); };
  const handlePrev = () => { if (chapter > 1) setChapter(c => c - 1); };

  const saveVerse = (text: string, verseNum: number) => {
      const reference = `${displayBookName} ${chapter}:${verseNum}`;
      onSaveItem({ id: uuidv4(), type: 'verse', content: text, reference: reference, date: Date.now() });
      setActiveVerse(null);
  };
    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 transition-colors">
       <header className="bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 p-4 sticky top-0 z-10 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
           <div className="flex items-center gap-2 w-full md:w-auto">
               <button onClick={onMenuClick} className="p-2 -ml-2 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg"><ArrowLeft size={24} /></button>
               <div className="bg-emerald-100 dark:bg-slate-800 p-2 rounded-lg text-emerald-600 dark:text-emerald-400"><Book size={20} /></div>
               <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100 font-serif-text">{displayBookName} {chapter}</h1>
               <button onClick={toggleAudio} disabled={loading || !data} className={`ml-2 p-2 rounded-full transition-all flex items-center justify-center ${isPlaying ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
                   {audioLoading ? <Loader2 size={20} className="animate-spin" /> : isPlaying ? <Pause size={20} fill="currentColor"/> : <Volume1 size={20}/>}
               </button>
           </div>
           <div className="flex items-center gap-2 w-full md:w-auto">
               <select value={selectedBookId} onChange={(e) => { setSelectedBookId(e.target.value); setChapter(1); }} className="flex-1 md:w-48 p-2 bg-slate-100 dark:bg-slate-800 border-none rounded-lg text-sm font-medium text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none">
                   <optgroup label={t.oldTestament}>{BIBLE_BOOKS.filter(b => b.testament === 'OT').map(b => (<option key={b.id} value={b.id}>{getLocalizedBookName(b)}</option>))}</optgroup>
                   <optgroup label={t.newTestament}>{BIBLE_BOOKS.filter(b => b.testament === 'NT').map(b => (<option key={b.id} value={b.id}>{getLocalizedBookName(b)}</option>))}</optgroup>
               </select>
               <select value={chapter} onChange={(e) => setChapter(Number(e.target.value))} className="w-20 p-2 bg-slate-100 dark:bg-slate-800 border-none rounded-lg text-sm font-medium text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none">
       ...
       {showBufferingWarning && (
           <div className="absolute top-20 left-1/2 -translate-x-1/2 z-30 bg-indigo-900/90 text-white px-4 py-2 rounded-full flex items-center gap-3 shadow-xl backdrop-blur-sm animate-scale-in">
               ...
               <div className="flex flex-col"><span className="text-xs font-bold">Downloading audio...</span><span className="text-[10px] text-indigo-300">Using high quality AI voice</span></div>
           </div>
       )}
           ...
           <div className="max-w-3xl mx-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm p-8 min-h-[60vh] mb-10">
               {loading ? (
                   <div className="flex flex-col items-center justify-center h-64 text-slate-400">
                       <div className="w-8 h-8 border-4 border-indigo-200 border-t-indigo-600 rounded-full animate-spin mb-4"></div><p>{t.loading}</p>
                   </div>
               ) : data ? (
                   <div className="font-serif-text leading-loose text-lg text-slate-800 dark:text-slate-200 pb-20">
                       <h2 className="text-3xl font-bold mb-6 text-center text-slate-900 dark:text-white border-b pb-4 border-slate-100 dark:border-slate-700">{displayBookName} {chapter}</h2>
                       {data.verses.map((v) => {
                           ...
                           return (
                               <span key={v.verse} onClick={() => setActiveVerse(activeVerse === v.verse ? null : v.verse)} className={`relative inline cursor-pointer rounded px-1 transition-all mx-0.5 duration-300 ${highlightColor || 'hover:bg-slate-100 dark:hover:bg-slate-700'} ${isReading ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-900 dark:text-indigo-100' : ''}`}>
                                   <sup className={`text-xs font-sans mr-1 select-none ${isReading ? 'text-indigo-500 font-bold' : 'text-slate-400'}`}>{v.verse}</sup>{v.text}
                                   {activeVerse === v.verse && (
                                       <span className="absolute -top-14 left-1/2 -translate-x-1/2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl p-1.5 flex gap-1 items-center z-20 whitespace-nowrap animate-scale-in">
                                           ...
                                           {highlightColor && (<button onClick={(e) => { e.stopPropagation(); removeHighlight(v.verse); }} className="w-6 h-6 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-500 hover:text-red-500"><X size={14} /></button>)}
                                           <div className="w-px h-4 bg-slate-200 dark:bg-slate-700 mx-1"></div>
                                           <button onClick={(e) => { e.stopPropagation(); onOpenComposer(v.text, `${displayBookName} ${chapter}:${v.verse}`); setActiveVerse(null); }} className="flex items-center gap-1 text-xs font-bold text-slate-600 dark:text-slate-300 hover:text-purple-600 px-2 py-1 hover:bg-slate-50 dark:hover:bg-slate-800 rounded"><Image size={14} /></button>
                                           <button onClick={(e) => { e.stopPropagation(); saveVerse(v.text, v.verse); }} className="flex items-center gap-1 text-xs font-bold text-slate-600 dark:text-slate-300 hover:text-indigo-600 px-2 py-1 hover:bg-slate-50 dark:hover:bg-slate-800 rounded"><Heart size={14} /> {t.save}</button>
                                       </span>
                                   )}
                                   {" "}
                               </span>
       ...
       <footer className="bg-white dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 p-4 flex items-center justify-between">
           <button onClick={handlePrev} disabled={chapter <= 1} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 disabled:opacity-50 text-slate-700 dark:text-slate-300 font-medium hover:bg-indigo-50 dark:hover:bg-slate-700 transition-colors"><ChevronLeft size={18} /> {t.prev}</button>
           <span className="text-sm text-slate-500 font-medium hidden md:block">{displayBookName} {chapter} / {selectedBook.chapters}</span>
           <button onClick={handleNext} disabled={chapter >= selectedBook.chapters} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 disabled:opacity-50 text-slate-700 dark:text-slate-300 font-medium hover:bg-indigo-50 dark:hover:bg-slate-700 transition-colors">{t.next} <ChevronRight size={18} /></button>
       </footer>
    </div>
  );
};
