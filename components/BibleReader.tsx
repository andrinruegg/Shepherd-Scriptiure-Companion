    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 transition-colors relative">
       <header className="bg-white dark:bg-slate-950 border-b dark:border-slate-800 p-4 sticky top-0 z-10 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
           <div className="flex items-center gap-2 w-full md:w-auto">
               ...
               <button onClick={handleAudioToggle} className={`ml-2 p-2 rounded-full transition-all ${isPlaying ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`} title={isPlaying ? t('bible.audio.pause') : t('bible.audio.play')}>{audioLoading ? <Loader2 size={20} className="animate-spin" /> : isPlaying ? <Pause size={20} fill="currentColor"/> : <Volume1 size={20}/>}</button>
           </div>
           <div className="flex items-center gap-2 w-full md:w-auto">
                                   ...
                                   {activeVerse === v.verse && (
                                       <span className="absolute -top-14 left-1/2 -translate-x-1/2 bg-white dark:bg-slate-900 border dark:border-slate-700 rounded-2xl shadow-2xl p-2 flex gap-1.5 items-center z-20 whitespace-nowrap animate-scale-in">
                                           {(['yellow', 'green', 'blue', 'pink'] as const).map((color) => (
                                              <button key={color} onClick={(e) => { e.stopPropagation(); onAddHighlight({id: uuidv4(), ref: verseRef, color}); setActiveVerse(null); }} className={`w-7 h-7 rounded-full ${getMarkerClass(color)} hover:scale-125 transition-transform shadow-sm border dark:border-white/20`} title={t(`bible.colors.${color}`)}></button>
                                           ))}
                                           <div className="w-px h-6 bg-slate-100 dark:bg-slate-700 mx-0.5"></div>
                                           <button onClick={(e) => { e.stopPropagation(); onOpenComposer(v.text, `${displayBookName} ${chapter}:${v.verse}`); setActiveVerse(null);}} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-purple-600 transition-colors" title={t('dailyVerse.createImage')}><Image size={16}/></button>
                                           <button onClick={(e) => { e.stopPropagation(); onSaveItem({id: uuidv4(), type:'verse', content:v.text, reference:`${displayBookName} ${chapter}:${v.verse}`, date:Date.now()}); setActiveVerse(null);}} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-rose-500 transition-colors" title={t('bible.save')}><Heart size={16}/></button>
                                           <button onClick={(e) => { e.stopPropagation(); onRemoveHighlight(verseRef); setActiveVerse(null); }} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 text-red-500 rounded-lg transition-colors" title={t('bible.colors.remove')}><X size={16}/></button>
                                       </span>
                                   )}
                               </span>
                           );
