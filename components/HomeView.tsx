        ...
        <div className="flex flex-col h-full overflow-y-auto bg-transparent transition-colors">
            ...
            <header className="px-4 md:px-6 py-4 md:py-5 flex items-center justify-between glass-header sticky top-0 z-50 bg-white/60 dark:bg-slate-950/70 backdrop-blur-2xl border-b border-white/50 dark:border-white/5 pt-safe">
                <div className="flex items-center gap-3">
                    <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-2 rounded-xl shadow-lg shadow-indigo-500/20 transform hover:scale-105 transition-transform">
                        {/* Fix: Removed invalid mdSize prop */}
                ...
                <div className="flex items-center gap-1 md:gap-2">
                    {/* Fix: Removed invalid mdSize prop from icons */}
                    <button onClick={onOpenSanctuary} className="p-2 md:p-2.5 rounded-full hover:bg-white/80 dark:hover:bg-white/10 text-slate-500 dark:text-slate-400 transition-all" title={tSidebar.tooltips?.sanctuary}>
                        <Headphones size={20} />
                    </button>
                    <button onClick={onOpenNotifications} className="p-2 md:p-2.5 rounded-full hover:bg-white/80 dark:hover:bg-white/10 text-slate-500 dark:text-slate-400 transition-all relative" title={tSidebar.tooltips?.inbox}>
                        <Bell size={20} />
                    ...
                    <button onClick={onOpenSettings} className="p-2 md:p-2.5 rounded-full hover:bg-white/80 dark:hover:bg-white/10 text-slate-500 dark:text-slate-400 transition-all" title={tSidebar.tooltips?.settings}>
                        <Settings size={20} />
                    </button>
                    <button onClick={onOpenProfile} className="ml-1 w-8 h-8 md:w-10 md:h-10 rounded-full bg-slate-200 dark:bg-slate-800 overflow-hidden border-2 border-white dark:border-slate-700 shadow-md transform hover:scale-110 transition-transform" title={tSidebar.tooltips?.profile}>
            ...
            <div className="flex-1 p-6 md:p-10 max-w-6xl mx-auto w-full pb-28 relative z-10">
                ...
                <div className="flex items-center gap-4 md:gap-6 mb-8 md:mb-10 ml-1 animate-slide-up">
                    <h3 className="text-[10px] md:text-[11px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.4em]">{t.jumpTo}</h3>
                        ...
                        <div className="bg-white/95 dark:bg-slate-900/95 h-full rounded-[1.8rem] md:rounded-[2.1rem] p-6 md:p-8 flex items-center justify-between backdrop-blur-2xl transition-all group-hover:bg-white/80 dark:group-hover:bg-slate-800">
                            <div className="flex items-center gap-4 md:gap-6">
                                ...
                                <div className="text-left">
                                    <h4 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white mb-0.5 md:mb-1">{t.chatTitle}</h4>
                                    <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400 font-medium">{t.chatDesc}</p>
                                </div>
                            </div>
                    ...
                    >
                        <div className="bg-white/95 dark:bg-slate-900/95 h-full rounded-[1.8rem] md:rounded-[2.1rem] p-6 md:p-8 flex items-center justify-between backdrop-blur-2xl transition-all group-hover:bg-white/80 dark:group-hover:bg-slate-800">
                            <div className="flex items-center gap-4 md:gap-6">
                                ...
                                <div className="text-left">
                                    <h4 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white mb-0.5 md:mb-1">{t.roleplayTitle}</h4>
                                    <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400 font-medium">{t.roleplayDesc}</p>
                                </div>
                            </div>
