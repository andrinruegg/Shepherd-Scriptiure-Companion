...
const App: React.FC = () => {
  ...
  const handleUpdatePreference = (key: keyof UserPreferences, value: any) => {
      ...
      if (key === 'theme') {
          ...
          if (isDark) {
              setPreferences(prev => ({ ...prev, princessTheme: false }));
          }
      }
  ...
  return (
    <div className={`${isDarkMode ? 'dark' : ''} animate-fade-in ${session ? 'h-[100dvh] overflow-hidden' : 'min-h-[100dvh]'} relative`}>
      ...
      {/* Overlays */}
      {preferences.winterTheme && <WinterOverlay showSnow={preferences.winterSnow} showLights={preferences.winterLights} showIcicles={preferences.winterIcicles} />}
      {preferences.princessTheme && <PrincessOverlay showHearts={preferences.princessHearts} showSparkles={preferences.princessSparkles} />}
        ...
        <Suspense fallback={<div className="h-full w-full flex items-center justify-center bg-stone-900 text-white">{t('common.loading')}</div>}>
            ...
            {currentView === 'stories' && (
                <RoleplayView 
                    ...
                    hasApiKey={hasApiKey} 
                    onTriggerKeyWarning={() => setIsSettingsOpen(true)}
                />
            )}
